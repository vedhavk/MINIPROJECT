...
export default function HealthMonitoring() {
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {vetHealthStats.map((stat) => {
          ...
          return (
            <div
              key={stat.id}
              className={`rounded-2xl border bg-white p-5 shadow-sm ${stat.accent.split(" ")[0]}`}
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-sm text-slate-500">{stat.label}</p>
                  <p
                    className={`mt-2 text-4xl font-bold ${stat.accent.split(" ")[1]}`}
                  >
                    {stat.value}
                  </p>
                </div>
                <div className={`rounded-xl p-3 ${stat.accent.split(" ")[2]}`}>
                  <Icon className={`h-7 w-7 ${stat.accent.split(" ")[1]}`} />
                </div>
              </div>
      ...
      <div className="grid grid-cols-4 rounded-full bg-white p-1 text-sm font-medium text-slate-700 shadow-sm">
        <div className="rounded-full bg-slate-100 px-4 py-2 text-center">
          Health Alerts
        </div>
        <div className="px-4 py-2 text-center">Individual Monitoring</div>
        <div className="px-4 py-2 text-center">Behavior Analysis</div>
        <div className="px-4 py-2 text-center">Image Review</div>
      </div>
      ...
      <section className="rounded-2xl bg-white p-5 shadow-sm">
        ...
        <div className="space-y-4">
          {vetAlerts.map((alert) => (
            <div
              key={alert.id}
              className={`flex flex-col gap-4 rounded-2xl border p-4 lg:flex-row lg:items-center lg:justify-between ${alert.border}`}
            >
              <div>
